...
//busca o id na url, se não estiver fazio, ele vai achar no banco
if(!empty($_GET['id'])) {  
    $data = $db->find($_GET['id']);
} 


if (!empty($_POST['id_catalogo'])) { //pega o id do formulario (POST) e trasnforma em int
    $filme_id = intval($_POST['id_catalogo']);
} elseif (!empty($data->id_catalogo)) { //procura no data o id do catalogo, se achar, o $filme-id recebe esse id
    $filme_id = $data->id_catalogo;
} elseif (!empty($_GET['id_catalogo'])) {
    $filme_id = intval($_GET['id_catalogo']); //pega pelo o id do url (GET)//intval transforma em num interirio
} else {
    // gera o erro se não houver id nenhum
...
<?php
    //Não está puxando corretamente do db, dai inseri elas aqui
                            ...
                            <h4 class="mb-4">
                                ...
                                <span class="text-dark fw-bold">
                                    ...
                                    if (!empty($catalogofilmes)): 
                                        foreach ($catalogofilmes as $itemFilme):  //percorre o catalogo e salva em $itemfilme
